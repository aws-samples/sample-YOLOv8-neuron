    ...
    with open('weights/step.csv', 'w') as f:
        ...
        for epoch in range(args.epochs):
            ...
            if args.local_rank == 0:
                ...
                if best == last[1]:
                    torch.save(ckpt, './weights/best-m.pt')
                del ckpt

    if args.local_rank == 0:
        util.strip_optimizer('./weights/best-m.pt')  # strip optimizers
...
def test(args, params, model=None):
    ...
    if model is None:
        # 修改模型加载
        # model = torch.load('./weights/best-m.pt', map_location='cuda')['model'].float()
        model = torch.load('./weights/best-m.pt', map_location=device)['model'].float()
...
def main():
    ...
    # parser.add_argument('--parallel-compile', action='store_true',
                      #help='enable parallel compile for neuron')
